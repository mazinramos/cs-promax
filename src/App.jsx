...
/* =================================================================================
   1. CSS STYLES (GLOBAL RESET & POLISHED DESIGN)
   ================================================================================= */
const customStyles = `
...
/* GLOBAL RESET - عشان نضمن الشكل واحد في كل الأجهزة */
*, *::before, *::after {
...
.main-content {
  flex: 1;
  width: 100%;
  max-width: 1280px; /* Laptop constraint */
  margin: 0 auto;
  padding: 24px 16px;
}
...
/* --- PANELS (POLISHED) --- */
.dark-panel {
  background: rgba(15, 15, 15, 0.96); /* Darker & clearer */
...
.dark-panel:hover {
  ...
  box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.8), 0 10px 10px -5px rgba(0, 0, 0, 0.04);
}

/* --- GRID SYSTEM (PERFECTED) --- */
.responsive-grid {
  display: grid;
  gap: 20px;
...
.animate-entry { animation: fadeIn 0.4s ease-out forwards; }
@keyframes pulse-gold { 0%, 100% { box-shadow: 0 0 0 rgba(255, 213, 79, 0); } 50% { box-shadow: 0 0 15px rgba(255, 213, 79, 0.3); } }
.animate-pulse-gold { animation: pulse-gold 3s infinite; }
`;

/* =================================================================================
   2. INTERACTIVE MATRIX BACKGROUND (FIXED LAYERING)
   ================================================================================= */
const InteractiveMatrix = () => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ctx.fillStyle = 'rgba(0, 0, 0, 0.08)'; // Trail
...
const LoginScreen = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative font-cairo z-20">
      <div className="w-full max-w-md dark-panel p-8 rounded-xl animate-entry border-t-4 border-t-[#FFD54F]">
        <div className="text-center mb-10">
          <div className="inline-flex p-4 rounded-full bg-black border border-[#FFD54F] mb-4 shadow-[0_0_20px_#FFD54F] animate-pulse-gold">
...
/* =================================================================================
   5. MAIN APPLICATION (Responsive & Polished)
   ================================================================================= */
export default function CsProMaxV22() {
  ...
  useEffect(() => { const saved = localStorage.getItem('cs_promax_v22_user'); if (saved) setUser(JSON.parse(saved)); }, []);
  const handleLogin = (data) => { setUser(data); localStorage.setItem('cs_promax_v22_user', JSON.stringify(data)); };
  const handleLogout = () => { localStorage.removeItem('cs_promax_v22_user'); setUser(null); setView('home'); };
      ...
      {!user ? (
        ...
        <>
          ...
          <nav className="sticky top-0 z-50 dark-panel border-b-0 border-b-[#3E2723] rounded-b-xl mx-2 md:mx-4 mt-2 shadow-2xl">
            <div className="main-content !py-2 !px-4 flex flex-col md:flex-row justify-between items-center gap-4 !max-w-full">
              ...
              <div className="animate-entry">
                ...
                <div className="responsive-grid">
                  {filteredSemesters.map((sem, index) => (
                    <div key={sem.id} onClick={() => {setActiveSemester(sem); setView('subjects')}} className="group relative dark-panel h-56 flex flex-col justify-between p-6 cursor-pointer">
                      ...
                      <div className="flex items-center gap-2 text-xs text-gray-500 font-code group-hover:text-white transition"><div className="w-2 h-2 bg-[#FFD54F] rounded-full animate-pulse-gold"></div> {sem.subjects.length} MODULES</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="animate-entry">
                <div className="dark-panel p-6 md:p-8 mb-8 relative overflow-hidden">
                   <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                     ...
                     <Cpu className="hidden md:block text-[#FFD54F] w-32 h-32 opacity-10 animate-pulse-gold"/>
                   </div>
                </div>
          ...
          <footer className="dark-panel mt-auto py-8 border-t border-t-[#3E2723] text-center rounded-none border-x-0 border-b-0"><p className="text-[#FFD54F] font-black text-lg tracking-widest font-cairo">CS PROMAX</p><p className="text-gray-600 text-xs font-code mt-2">SECURE_ARCHIVE_SYSTEM_V22.0</p></footer>
        </>
      )}
      </div>
    </div>
