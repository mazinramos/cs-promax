...
/* =================================================================================
   1. CSS STYLES (RESPONSIVE LAYOUT & FIXES)
   ================================================================================= */
const customStyles = `
...
@media (min-width: 640px) { .grid-system { grid-template-columns: repeat(2, 1fr); } }
...
@keyframes pulse-gold { 0%, 100% { box-shadow: 0 0 5px rgba(255, 213, 79, 0.2); } 50% { box-shadow: 0 0 20px rgba(255, 213, 79, 0.5); } }
...
const LoginScreen = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative font-cairo z-20">
      <div className="w-full max-w-md dark-panel p-8 rounded-xl animate-slide-up border-t-2 border-t-[#FFD54F]">
...
/* =================================================================================
   5. MAIN APPLICATION
   ================================================================================= */
export default function CsProMaxFinalDeploy() {
  ...
  useEffect(() => { const saved = localStorage.getItem('cs_promax_deploy_user'); if (saved) setUser(JSON.parse(saved)); }, []);
  const handleLogin = (data) => { setUser(data); localStorage.setItem('cs_promax_deploy_user', JSON.stringify(data)); };
  const handleLogout = () => { localStorage.removeItem('cs_promax_deploy_user'); setUser(null); setView('home'); };
      ...
      {!user ? (
        ...
        <>
          ...
          <main className="container-max py-8">
            {view !== 'home' && (
               <div className="flex items-center gap-2 mb-6 text-sm text-gray-400 font-code animate-slide-up">
